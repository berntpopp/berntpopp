...
export class SEOMonitor {
  ...
  analyzePerformance() {
    ...
    Object.entries(pages.pages).forEach(([path, page]) => {
      ...
      if (descLength < 120) {
        metrics.warnings.push(`${path}: Description too short (${descLength} chars). Aim for 150-160.`);
      } else if (descLength > 160) {
        metrics.warnings.push(`${path}: Description too long (${descLength} chars). Keep under 160.`);
      } else {
        metrics.successes.push(`${path}: Description length optimal (${descLength} chars).`);
      }
      
      if (!page.keywords || page.keywords.length < 3) {
        metrics.recommendations.push(`${path}: Add more keywords (current: ${page.keywords?.length || 0})`);
      }
    });
  ...
  analyzeTechnicalSEO(metrics, health) {
    ...
    // Check page coverage
    const pagesWithoutSEO = Object.entries(health.pages).filter(([_, data]) => {
